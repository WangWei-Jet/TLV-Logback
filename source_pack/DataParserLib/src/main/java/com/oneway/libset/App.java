...
public class App {
	...
	private static CustomLogger logger = CustomLogger.getLogger(App.class.getSimpleName());
	...
	public static void main(String[] args) {
		...
		// testStandardTlvParser_Parse("FF90000401102101");
		testCommonTlvParser_Parse("9F260101");
		// commonTest();
		testFrame();
	}
	...
	private static TLVParseResult testCommonTlvParser_Parse(String data) {
		TlvParser tlvParser = ParserFactory.getCommonTlvParser();
		long start = System.currentTimeMillis();
		// testData += "154154";
		tlvParser.setPreTags(new String[] { "9F26", "9F27", "9F28" });
		TLVParseResult result = tlvParser.parse(data);
		long end = System.currentTimeMillis();
		if (result != null) {
			logger.debug("解析结果:" + result.getParseResult() + "\n耗时:" + (end - start) + " ms");
	...
	private static void testFrame() {
		...
		bu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (result != null) {
					if (result.getTlvList() != null && result.getTlvList().size() > 0) {
